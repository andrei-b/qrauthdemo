    ...
    public class LoginUtil
    {
        ...
        static public int newLogin()
        {
            ...
            using (LiteDatabase db = new LiteDatabase(dbPath))
            {
                ...
                Int32 maxSession = logins.Max();
                maxSession += (new Random()).Next(256);
                UserSession usession = new UserSession(maxSession); 
                try
                {
                        // The following line raises an exception but still does what it should do. Buggy DBLite probably
                        logins.Insert(usession);
                }
                catch (InvalidCastException e)
                {
                }
